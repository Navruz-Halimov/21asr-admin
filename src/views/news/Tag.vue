    ...
    <div>
        <CContainer fluid>
           ...
           <CRow v-if="!$store.state.showLoader">
               <CCol col="12">
                   <Table :theads="thead" :tbodies="tags" :button="bshow" :objects="objs" @getItemEdit="getEdit" />
                    <CModal
                        title="O'zgartirish"
                        color="warning"
...
export default {
    ...
    methods: {
        ...
        getEdit(val) {
            ...
            .then(res => {
                this.tagsArr = this.$store.state.singleItemEdit;
            })
        },
        update() {
            this.loader = true;
            if(this.tagsArr.name && this.tagsArr.name_kl){
                this.$store.dispatch('update', { url: `${this.$store.state.currentItemUrl}/${this.itemId}/`, value: this.tagsArr } )
                .then(() => {
                    this.loader = false;
                })
            }else{
        ...
        add() {
            ...
            if(this.form.name && this.form.name_kl) {
                this.$store.dispatch('add',{ url: 'tags/', value: this.form })
                .then(() => {
